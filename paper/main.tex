% ============================================================
%  Bayesian Analysis submission — official imsart template
...
as introduced by \citet{schetinin2025}.
...
the $N=40$ knee osteoarthritis dataset of \citet{jakaite2021}.
...
structure. The model of \citet{schetinin2025}, hereafter JBDT, places a
...
Applied to the \citet{jakaite2021} knee osteoarthritis (OA) dataset,
...
the proposal \citep{schetinin2025}:
\begin{itemize}
...
The WAIC values from \citet{schetinin2025} give per-observation advantages
...
with 500 replicates.
The simulated total gap $\waic_{\rm dev} - \loo_{\rm dev}$ converges to $-4$ as $N\to\infty$,
closely tracking the formula $-4N/(N+\alpha_0)$ (maximum deviation $0.03$).
The per-observation gap follows the $1/N$ decay.
...
The PAC-Bayes complexity penalty versus $N$ for $k^*=1,2,3$
under Catalan and Chipman (numerical output available in the supplementary code)
...
feature orders \citep{jakaite2021} by treating each Zernike configuration
as a model family.
